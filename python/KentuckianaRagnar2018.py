...
docsdir = os.path.abspath(os.path.join(scriptdir, '../docs/races/2018/KentuckianaRagnar/'))

def deltaTimeToStr(diff):
    symbol = '-'
    try:
        diff = diff.to_timedelta64()
    except:
        pass # assume it is already the right object
    if diff < np.timedelta64(0, 's'):
        symbol = '+'
        diff *= -1
    hours = diff / np.timedelta64(1,'h')
    minutes = np.abs((diff - (hours * np.timedelta64(1,'h')))) / np.timedelta64(1, 'm')
    return '%s%dh%02dm' % (symbol, hours, minutes)

######################################################################
############################## calculate everything
# setup list of legs
legs = np.array([3.7, 4.3, 6.9]) ##### TODO #####
#print('short', legs)
...
runners = pd.DataFrame({'runner':np.arange(5),
                        'name':['NA', 'Mike', 'Peter', 'Pete', 'Dan'],
                        'pace':[np.timedelta64(int(0), 's')*1.20,
                                np.timedelta64(int(10*60), 's')*1.20,
                                np.timedelta64(int(9*60+30), 's')*1.20,
                                np.timedelta64(int(9*60+30), 's')*1.20,
...
with open(os.path.join(scriptdir, 'KentuckianaRagnar2018.json'), 'r') as handle: ##### TODO #####
    config = json.load(handle)
    for item in config['annotations']:
        key = list(item.keys())[0]
        item[key] = pd.Timestamp(item[key])
...
def magic(race, label):
    values = race[label][mask] # .values
    #print(label, type(values), values.shape, values)
    return values
...
for x, y, leg, act, est in \
    ...
    try:
        diff = deltaTimeToStr(y-est.to_datetime64())
    except:
